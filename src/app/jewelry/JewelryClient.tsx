...
const Jewelry = ({ products }: { products: UIProduct[] }) => {
  const jewelryProducts = products.filter((p) => p.category === "jewelry");
  const cancerCollection = jewelryProducts.filter((p) =>
    /cancer/i.test(p.name) || /cancer/i.test(p.slug)
  );
  const herkimerCollection = jewelryProducts.filter((p) =>
    /herkimer/i.test(p.name) || /herkimer/i.test(p.slug)
  );
  return (
    <>
...
const JewelryCard = ({
  ...
  return (
    ...
    >
      ...
      <Link href={`/product/${slug}`} className="block mt-5 space-y-1.5">
